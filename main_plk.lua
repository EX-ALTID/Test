...
package.path = package.path .. ";/savedata0/main_plk/?.lua"
...
function main()
    ...
    send_ps_notification(string.format("PS4 Lua Loader Memek \n %s %s", PLATFORM, FW_VERSION))


    if PLATFORM ~= "ps4" then
        notify(string.format("This only works on ps4 (current %s %s)", PLATFORM, FW_VERSION))
        return
    end
    ...
    send_ps_notification("PS4 Lua Loader finished!\n\nTutup Permainan...")
    syscall.kill(syscall.getpid(), 15)
end
